...
const getUsageData = (): UsageData => {
  if (typeof window === "undefined") return { tasksCompleted: 0, lastResetDate: getTodayDateString() };
  ...
  try {
    ...
    // Validate Checksum
    if (parsed.checksum !== generateChecksum({ tasksCompleted: parsed.tasksCompleted, lastResetDate: parsed.lastResetDate })) {
      throw new Error("Checksum mismatch");
    }

    const data: UsageData = { tasksCompleted: parsed.tasksCompleted, lastResetDate: parsed.lastResetDate };
