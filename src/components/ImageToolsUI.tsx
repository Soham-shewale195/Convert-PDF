    ...
    async (e: React.DragEvent) => {
      ...
      for (const f of fs) {
        if (await validateMagicNumbers(f, ["image"])) {
          ...
          toast.error(
            `Invalid file signature for ${f.name}. File does not appear to be a real image.`,
          );
        }
      }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <motion.div
          ...
          transition={
            over ? { duration: 0.2 } : { duration: 3, repeat: Infinity, ease: "easeInOut" }
          }
          ...
          >
            {files.map((f, i) => (
              <motion.li
                ...
                className="flex items-center gap-3 p-3 rounded-xl glass border border-white/10 group hover:bg-white/[0.04] transition-colors"
              >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-muted-foreground hover:bg-destructive/20 hover:text-red-400 transition-colors"
                  aria-label="Remove"
                >
                  <X className="w-4 h-4" />
                </button>
...
export function ImagePreview({
  ...
  return (
    <motion.div
      ...
      className="mt-5 rounded-2xl glass p-3 overflow-hidden border border-white/10 relative shadow-[0_8px_30px_rgba(0,0,0,0.12)]"
    >
