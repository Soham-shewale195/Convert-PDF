...
export default function WatermarkImagePanel() {
  ...
  const getPosStyle = () => {
    ...
    if (position.includes("top")) s.top = 0;
      ...
      s.top = "50%";
      s.transform = s.transform ? s.transform + " translateY(-50%)" : "translateY(-50%)";
    }
    ...
    if (position.includes("left")) s.left = 0;
      ...
      s.left = "50%";
      s.transform = s.transform ? s.transform + " translateX(-50%)" : "translateX(-50%)";
    }
  ...
  return (
    <>
      ...
      {files[0] && (
        <>
          ...
          <div className="mt-5 p-5 glass rounded-2xl border border-white/5 space-y-4">
            <div>
              <label className="block text-xs text-muted-foreground uppercase tracking-wider mb-1.5 font-semibold">
                Watermark Text
              </label>
              <input
                value={text}
                onChange={(e) => setText(e.target.value)}
                className="w-full glass rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/50 transition-all"
              />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-primary h-2 rounded-full appearance-none bg-white/10 shadow-[0_0_10px_rgba(139,92,246,0.2)]"
                />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground uppercase tracking-wider mb-1.5 font-semibold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full glass rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/50 [&>option]:bg-zinc-900"
                >
      ...
      {state === "success" ? (
        ...
        <ActionButton onClick={run} state={state} disabled={!files.length || !text.trim()}>
          Add Watermark
        </ActionButton>
      )}
      {renderModal()}
